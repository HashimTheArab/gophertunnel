...
type LevelChunk struct {
	...
	BlobHashes []protocol.SubChunkMetadata
...
func (pk *LevelChunk) Marshal(io protocol.IO) {
	...
	io.Varuint32(&pk.Dimension)
	protocol.Maximum(io, &pk.Dimension, 64)
	...
	protocol.SliceLimits(io, &pk.BlobHashes, 0, 65)
	io.Bytes(&pk.RawPayload)
}
