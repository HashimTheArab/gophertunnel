...
type StartGame struct {
	...
	// this field.
	EntityUniqueID protocol.GameType
	...
	Blocks []protocol.ServerBlockProperty
	...
	ServerJoinInformation protocol.Optional[protocol.ServerConfigurationServerConfigurationJoinInfo]
...
func (pk *StartGame) Marshal(io protocol.IO) {
	...
	pk.EntityUniqueID.Marshal(io)
	io.Vec3(&pk.PlayerPosition)
	io.Vec2(&pk.WorldSpawn)
	pk.EducationSharedResourceURI.Marshal(io)
	io.String(&pk.LevelID)
	io.String(&pk.WorldName)
	io.String(&pk.TemplateContentIdentity)
	io.Bool(&pk.Trial)
	pk.PlayerMovementSettings.Marshal(io)
	io.Uint64(&pk.Time)
