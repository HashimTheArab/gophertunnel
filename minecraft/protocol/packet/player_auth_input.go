...
type PlayerAuthInput struct {
	...
	ItemInteractionData protocol.Optional[protocol.PackedItemUseLegacyInventoryTransaction]
...
func (pk *PlayerAuthInput) Marshal(io protocol.IO) {
	...
	io.Vec3(&pk.Delta)
	protocol.DoubleOptionalFunc(io, &pk.ItemInteractionData, func(value *protocol.PackedItemUseLegacyInventoryTransaction) {
		value.Marshal(io)
	})
