...
func (nopCompression) CompressAppend(dst, decompressed []byte) ([]byte, error) {
...
func (snappyCompression) CompressAppend(dst, decompressed []byte) ([]byte, error) {
	// EncodeSnappy requires dst and decompressed not to overlap. Encoder passes
	// a separate output buffer here so the batch header, compression ID and
	// compressed payload can share one backing array.
