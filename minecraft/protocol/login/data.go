...
type ClientData struct {
	...
	PartyLeader bool `json:"IsPartyLeader,omitempty"`
	...
	// ExtraData holds client-data claims that this version of gophertunnel does not model. Claims are kept
	// here when decoding so proxies can forward additions made by newer clients without dropping them.
	ExtraData map[string]json.RawMessage `json:"-"`
	// ClaimOrder records the original JSON claim order when ClientData was decoded. A nil ClaimOrder means
	// that the value was constructed programmatically and should use the current vanilla claim order.
	ClaimOrder []string `json:"-"`
}

// clientDataClaimOrder follows the order used by the vanilla Bedrock client. JSON object order does not
// affect decoding, but retaining it keeps generated client-data JWTs byte-compatible with vanilla.
var clientDataClaimOrder = []string{
	"AnimatedImageData",
	"ArmSize",
	"CapeData",
	"CapeId",
...
var optionalClientDataClaims = []string{
	"PlatformUserId",
	"PlayFabId",
	"ThirdPartyNameOnly",
	"PartyId",
...
// MarshalJSON encodes ClientData using the claim order emitted by the vanilla Bedrock client.
func (data ClientData) MarshalJSON() ([]byte, error) {
	type clientData ClientData
	encoded, err := json.Marshal(clientData(data))
	if err != nil {
		return nil, err
	}
	...
	for name, value := range data.ExtraData {
		fields[name] = value
	}
	...
	order := clientDataClaimOrder
	preservePresence := data.ClaimOrder != nil
	if preservePresence {
		order = data.ClaimOrder
	}
	for _, name := range order {
		writeField(name)
	}
	if preservePresence {
		for _, name := range optionalClientDataClaims {
			writeField(name)
		}
	}
	...
	for name := range fields {
		...
		if _, ok := data.ExtraData[name]; ok {
			remaining = append(remaining, name)
		}
	}
...
func (data *ClientData) UnmarshalJSON(encoded []byte) error {
	type clientData ClientData
	var decoded clientData
	if err := json.Unmarshal(encoded, &decoded); err != nil {
		return err
	}
	order, err := clientDataObjectKeys(encoded)
	if err != nil {
		return err
	}
	fields := map[string]json.RawMessage{}
	...
	known := make(map[string]struct{}, len(clientDataClaimOrder))
	for _, name := range clientDataClaimOrder {
		known[name] = struct{}{}
	}
	...
	*data = ClientData(decoded)
	data.ExtraData = extra
	data.ClaimOrder = order
	return nil
}

func clientDataObjectKeys(encoded []byte) ([]string, error) {
	...
	if token != json.Delim('{') {
		return nil, fmt.Errorf("expected client data JSON object")
	}
	var keys []string
	for decoder.More() {
		token, err := decoder.Token()
